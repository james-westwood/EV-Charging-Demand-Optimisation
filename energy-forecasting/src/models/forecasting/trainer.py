...
def train_lgbm_no_mlflow(X: pd.DataFrame,
      y: pd.Series,
      alpha: float,
      n_splits: int = 5,
      gap: int = 48,
    ) -> tuple[Booster, np.ndarray]:
    """Trains the lgbm model without mlflow

    The no mlflow version is needed on the community version 
    of Databricks because the node workers can't access the mlflow server
    ...
    # Log oof pinball loss ignoring nans
    mask = ~np.isnan(oof_preds)
    pb_loss = pinball_loss(alpha, oof_preds[mask], y.values[mask])

    return final_model, oof_preds, pb_loss   # 3. return the now-trained model  
